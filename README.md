...
## Cost protection & caching (transcript abuse control)

The transcript endpoint guards variable-cost APIs at the application level
(Cloudflare is the first line of defense; this is the backstop). See
`src/lib/rate-limit.ts` and `src/lib/cache.ts`.

- **Cache-first:** `/api/transcript` returns a cached transcript (keyed by
  videoId + language, TTL `TRANSCRIPT_CACHE_TTL_DAYS`) **before** spending any
  rate-limit quota or calling the paid provider.
- **Tiered limits:** per-IP hourly (`TRANSCRIPT_IP_HOURLY_LIMIT`), per-IP daily
  (`TRANSCRIPT_IP_DAILY_LIMIT`), and a global daily cap
  (`TRANSCRIPT_GLOBAL_DAILY_LIMIT`). Over-limit requests get a friendly
  "try again later" 429 (never a stack trace) and are written to an abuse log.
- **In-memory, per-instance** for v1 — swap for Redis/Upstash for multi-instance.

## Cloudflare Turnstile (optional bot challenge)

Off by default (`ENABLE_CLOUDFLARE_TURNSTILE=false`) so normal users never see a
challenge. When enabled, the transcript/summary routes verify a Turnstile token
server-side (`src/lib/turnstile.ts`). Set
`CLOUDFLARE_TURNSTILE_SECRET_KEY` + `NEXT_PUBLIC_CLOUDFLARE_TURNSTILE_SITE_KEY`.

## PWA (installable)

LinkTranscript is an installable PWA. Manifest is generated by
`src/app/manifest.ts`; icons live in `public/icons/` (192, 512, maskable 512,
apple-touch 180). Theme/background color, viewport, and Apple status-bar styling
are set in `src/app/layout.tsx`. Users can add it to a phone home screen and open
it like an app. (No native iOS/Android apps.)

## Summaries (Phase 2 — optional, not built in v1)

Reserved env vars exist (`ENABLE_SUMMARIES`, `AI_PROVIDER`, provider keys,
`SUMMARY_*` limits). When built, summaries will be **server-side only** (AI keys
never reach the browser), rate-limited, cached by videoId + transcript hash, and
hidden when `ENABLE_SUMMARIES=false`. The core transcript tool always works
without AI.
